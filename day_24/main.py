...
def add_map():
    ...
    map_bank[map_bank['last']] = move_map(
        deepcopy(map_bank[map_bank['last'] - 1]), 
        deepcopy(map_bank['clean'])
        )
        ...
        if (c <= max_c) and (r <= max_r) and (c >= 0) and (r >= 0):
            if map_bank[t + 1][r][c] == '.':
                if (r, c) not in move_bank[t + 1]:
                    ...
                    min_time = min(
                        min_time, 
                        search_path(t + 1, r, c, max_r, max_c, min_time)
                        )
...
def solve(data_path="input"):
    ...
    time_min_2, last_map = find_shortest(
        rmap(deepcopy(last_map)), deepcopy(data_clean)
        )
        
    # Search for the shortest path from start to end
    time_min_3, _ = find_shortest(
        rmap(deepcopy(last_map)), deepcopy(data_clean)
        )
