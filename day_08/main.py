...
def part1(data_path="input"):
    ...
    for i in range(1, r):
        ...
        down[r - 1 - i, :] = np.multiply(down[r - 1 - i, :], down[r - 1 - i, :] > down[r - 1, :])
    ...
    for i in range(1, c):
        ...
        right[:, c - 1 - i] = np.multiply(right[:, c - 1 - i], right[:, c - 1 - i] > right[:, c - 1])
...
def part2(data_path="input"):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            check_dir = {'up': r > 0, 'down': r < rows - 1, 'left': c > 0, 'right': c < cols - 1} 
            i = 0
